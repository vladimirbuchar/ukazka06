...
{
    public class SendEmailJob(IServiceScopeFactory serviceScopeFactory) : Core.Base.HangfireJob.Hangfire(serviceScopeFactory)
    {
        public override void Execute()
        {
            ...
            foreach (SendEmailDbo sendEmail in sendEmails)
            {
                try
                {
                    OrganizationSettingDbo getOrganizationSetting = _organizationSettingRepository
                        .GetEntity(false, x => x.OrganizationId == sendEmail.OrganizationId)
                        .Result;
