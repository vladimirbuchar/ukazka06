...
namespace EduServices.SendMessage.Validator
{
    ...
    {
        private readonly HashSet<SendMessageTypeDbo> _sendMessageTypes = codeBookRepository.GetCodeBookItems();
        ...
        public override Result<SendMessageDetailDto> IsValid(SendMessageCreateDto create)
        {
            ...
            IsValidString(create.Name, result, ErrorCategory.SEND_MESSAGE, GlobalValue.STRING_IS_EMPTY);
        ...
        {
            Result<SendMessageDetailDto> result = new();
            IsValidString(update.Name, result, ErrorCategory.SEND_MESSAGE, GlobalValue.STRING_IS_EMPTY);
        ...
        private void IsValidMessageType(Guid messageTypeId, string email, Result result)
        {
            ...
            {
                result.AddResultStatus(new ValidationMessage(MessageType.ERROR, ErrorCategory.SEND_MESSAGE, Constants.SELECT_MESSAGE_TYPE));
            }
            if (_sendMessageTypes.FirstOrDefault(x => x.Id == messageTypeId).SystemIdentificator == SendMessageType.EMAIL)
            {
                ...
                {
                    result.AddResultStatus(new ValidationMessage(MessageType.ERROR, ErrorCategory.SEND_MESSAGE, Constants.REPLY_EMAIL_IS_EMPTY));
                }
                if (!email.IsValidEmail())
                {
                    result.AddResultStatus(new ValidationMessage(MessageType.ERROR, ErrorCategory.SEND_MESSAGE, Constants.REPLY_EMAIL_IS_NOT_VALID));
                }
            }
        }
    }
