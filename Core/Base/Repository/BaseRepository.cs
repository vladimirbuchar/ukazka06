...
namespace Core.Base.Repository
{
        ...
        where Model : TableModel
    {
        ...
        protected IQueryable<Model> PrepareOrderBy(IQueryable<Model> query, List<BaseSort<Model>> orderBy = null)
        {
            if (orderBy != null && orderBy.Count > 0)
            {
                IOrderedQueryable<Model> orderedQuery =
                    orderBy[0].SortDirection == SortDirection.Ascending ? query.OrderBy(orderBy[0].Sort) : query.OrderByDescending(orderBy[0].Sort);
                for (int i = 1; i < orderBy.Count; i++)
                {
                    orderedQuery =
                        orderBy[i].SortDirection == SortDirection.Ascending
                            ? orderedQuery.ThenBy(orderBy[i].Sort)
                            : orderedQuery.ThenByDescending(orderBy[i].Sort);
                }
                query = orderedQuery;
            }
            return query;
        ...
        protected virtual IQueryable<Model> PrepareDropDownQuery()
        {
            return _dbContext.Set<Model>();
        }
        ...
        public virtual async Task<List<Model>> GetEntitiesDropDown(
            bool deleted,
            Expression<Func<Model, bool>> predicate = null,
            List<BaseSort<Model>> orderBy = null,
            Expression<Func<Model, bool>> customPredicate = null
        ...
        {
            IQueryable<Model> query = PrepareDropDownQuery();
            query = PrepareWhere(query, deleted, predicate, customPredicate);
            query = PrepareOrderBy(query, orderBy);
            return await query.ToListAsync();
        }
