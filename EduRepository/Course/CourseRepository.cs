            ...
            ICourseRepository
    {
        ...
        protected override IQueryable<CourseDbo> PrepareListQuery()
        {
            return _dbContext
                ...
                .ThenInclude(x => x.SendMessageTranslations.Where(x => x.IsDeleted == false))
                .ThenInclude(x => x.Culture)
                .Include(x => x.Certificate)
                .ThenInclude(x => x.CertificateTranslations.Where(x => x.IsDeleted == false))
                .ThenInclude(x => x.Culture)
                .Include(x => x.CourseMaterial)
                .ThenInclude(x => x.CourseMaterialTranslation.Where(x => x.IsDeleted == false))
                .ThenInclude(x => x.Culture);
        }
