...
namespace Repository.CourseMaterial
{
            ...
            ICourseMaterialRepository
    {
        ...
        {
            return (await _dbContext.Set<CourseMaterialFileRepositoryDbo>().Include(x => x.CourseMaterial).FirstOrDefaultAsync(x => x.Id == objectId))
                .CourseMaterial
                .OrganizationId;
        }

        protected override IQueryable<CourseMaterialDbo> PrepareDropDownQuery()
        {
            return _dbContext.Set<CourseMaterialDbo>().Include(x => x.CourseMaterialTranslation.Where(x => x.IsDeleted == false)).ThenInclude(x => x.Culture);
        }
    }
}
