...
namespace Repository.BranchRepository
{
    ...
    {
        public override HashSet<BranchDbo> GetEntities(
            bool deleted,
            Expression<Func<BranchDbo, bool>> predicate = null,
            Expression<Func<BranchDbo, object>> orderBy = null,
            Expression<Func<BranchDbo, object>> orderByDesc = null
        )
        {
            return [.. _dbContext.Set<BranchDbo>().Include(x => x.BranchTranslations.Where(x => x.IsDeleted == false)).ThenInclude(x => x.Culture).Where(predicate).Where(x => x.IsDeleted == deleted)];
        }
